...
%global _privatelibs lib(openblas_|openblas64_|ccalltest|llvmcalltest|LLVM-.*|uv|blastrampoline|unwind)\\.so.*
...
# Work around https://github.com/staticfloat/libblastrampoline/issues/44
# and https://github.com/staticfloat/libblastrampoline/issues/50
# -Wno-error is needed to cancel -Werror passed upstream, see
# https://github.com/JuliaLinearAlgebra/libblastrampoline/pull/81
%if 0%{?__isa_bits} == 64
make %{?_smp_mflags} CFLAGS="%{build_cflags} -fPIC -DLIBRARY_EXPORTS -DF2C_AUTODETECTION -Wno-error" LDFLAGS="%{build_ldflags} -shared" %{commonopts} -C deps install-blastrampoline
%else
make %{?_smp_mflags} CFLAGS="%{build_cflags} -fPIC -DLIBRARY_EXPORTS -Wno-error" LDFLAGS="%{build_ldflags} -shared" %{commonopts} -C deps install-blastrampoline
%endif
