...
%if %{with lua}
%global hsluacli hslua-cli-1.4.2
...
%global hsluamodulezip hslua-module-zip-1.1.1
...
%global subpkgs %{lpeg} %{hsluarepl} %{hsluamoduleversion} %{hsluamodulesystem} %{hsluamodulepath} %{hsluamoduledoclayout} %{hslualist} %{hsluamodulezip} %{pandocluamarshal} %{pandocluaengine} %{hsluacli}
%endif
...
Source0:        https://hackage.haskell.org/package/%{name}-%{pandoccli_ver}/%{name}-%{pandoccli_ver}.tar.gz
%if %{with lua}
...
Provides:       pandoc-cli = %{pandoccli_ver}-%{release}
...
%setup -q -n %{name}-%{pandoccli_ver} %{?with_lua:-a1 -a2 -a3 -a4 -a5 -a6 -a7 -a8 -a9 -a10 -a11}
# End cabal-rpm setup
%if %{without lua}
